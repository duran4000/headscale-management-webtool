# Install SSH Tunnel Scheduled Task (IPv6)
# Requires admin privileges

$TaskName = "SSH-Tunnel-Headscale"
$ScriptPath = Join-Path $PSScriptRoot "ssh-tunnel-headscale-ipv6.ps1"

Write-Host "==========================================" -ForegroundColor Cyan
Write-Host "SSH Tunnel Task Installer (IPv6)" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
...
# Create task with AtLogon trigger (not AtStartup)
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Principal $principal -Settings $settings -Description "SSH Tunnel to Headscale Server (IPv6) for bypassing GFW" | Out-Null
...
try {
    ...
    $tcpClient.Connect([System.Net.IPAddress]::IPv6Loopback, 8443)
    $tcpClient.Close()
    Write-Host "Tunnel connected! Local [::1]:8443 is ready." -ForegroundColor Green
} catch {
    Write-Host "Tunnel connection failed: $_" -ForegroundColor Yellow
    Write-Host "Check log: $PSScriptRoot\ssh-tunnel-ipv6.log" -ForegroundColor Yellow
}
...
Write-Host "  Log:     Get-Content '$PSScriptRoot\ssh-tunnel-ipv6.log' -Tail 50" -ForegroundColor Gray
